...
# ─── Config ────────────────────────────────────────────────────────────────
# Embedding provider: OpenRouter by default, or any OpenAI-compatible endpoint
# To use Qwen official API, set in .env:
...
EMBEDDING_KEY = EMBEDDING_API_KEY or OPENROUTER_KEY or ""
# Normalize: DashScope compatible-mode REQUIRES /v1 in path.
# Without this, embedding returns 404 and all qdrant_search calls fail silently.
# This guards against EMBEDDING_BASE_URL set without /v1 (e.g. from .env).
_base = EMBEDDING_BASE_URL or "https://dashscope.aliyuncs.com/compatible-mode/v1"
if _base and "/v1" not in _base:
    _base = _base.rstrip("/") + "/v1"
EMBEDDING_URL = _base + "/embeddings"
...
def embed_query(text: str) -> Optional[List[float]]:
    """Generate dense embedding via configurable provider (OpenRouter or Qwen official)."""
...
"""],
        ...
        # FastEmbed not available — expected in most deployments. Dense-only works fine.
        return None
...
def search_knowledge_base(
    ...
    try:
        ...
        for r in points:
            ...
            # Support both migration formats:
            # - memory-os: {text, title, source, tags}
            # - hermes_memory_migration: {content, category, tag, source}
            text = payload.get("text") or payload.get("content") or ""
            title = payload.get("title") or payload.get("category") or payload.get("tag") or "Untitled"
            tags = payload.get("tags", [])
            if not tags and payload.get("tag"):
                tags = [payload["tag"]]
            results.append({
                "id": r.get("id", "unknown"),
                "score": score,
                "title": title,
                "content_preview": _strip_prompt_injection(text[:400]),
...
def tokenize_query(text: str) -> List[str]:
    ...
    # Extract Latin + CJK tokens for lexical fallback
    latin = re.findall(r'\b[a-zA-Z0-9]+\b', text.lower())
    cjk_chars = re.findall(r'[\u4e00-\u9fff\u3400-\u4dbf\uf900-\ufaff]', text)
    cjk_bigrams = {cjk_chars[i] + cjk_chars[i+1] for i in range(len(cjk_chars)-1)}
    tokens = latin + [c for c in cjk_chars if c not in stopwords] + list(cjk_bigrams)
    # Extended stopwords with Chinese
    zh_stopwords = {"的", "了", "是", "在", "我", "有", "和", "就", "不", "人",
                    "都", "一", "个", "上", "也", "很", "到", "说", "要", "去"}
    return [t for t in tokens if t not in stopwords and t not in zh_stopwords and len(t) > 2]
...
def search_with_fallback(
    ...
    try:
        ...
        if sparse_vector is not None:
            resp = requests.post(
                ...
                json={
                    "prefetch": [
                        {"query": {"dense": dense_vector}, "limit": top_k * 3},
                        {"query": {"sparse": {"indices": sparse_vector[0], "values": sparse_vector[1]}},
                         "limit": top_k * 3},
                    ],
                    "query": {"fusion": "rrf"},
                    "limit": top_k * 2,
    ...
    # Nothing worked
    fallback_latency_ms = (time.perf_counter() - t0) * 1000
    # No results from any level — expected when query doesn't match stored content.
